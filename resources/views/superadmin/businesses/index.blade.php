                ...
                @forelse($businesses as $business)
                    <tr>
                        ...
                        <td class="fw-bolder text-gray-500">
                            ...
                            @if($business->rfc)
                                <div class="small text-gray-400">RFC: {{ $business->rfc }}</div>
                            @endif
                        </td>
                        ...
                        <td>
                            @if($business->plan)
                                ...
                                <div class="small text-gray-400">${{ number_format($business->plan->price, 2) }}/mes</div>
                            @else
                                <span class="fw-bold text-warning">Sin plan</span>
                            @endif
                        </td>
                        <td class="text-gray-500">
                            {{ $business->registration_date?->format('d/m/Y') ?? 'N/A' }}
                        </td>
                        <td>
                                ...
                                <div class="dropdown-menu dashboard-dropdown dropdown-menu-end mt-2 py-1">
                                    ...
                                    <a class="dropdown-item d-flex align-items-center {{ $business->is_active ? 'text-danger' : 'text-success' }}" href="#"
                                       onclick="event.preventDefault(); document.getElementById('toggle-form-{{ $business->business_id }}').submit();">
                                        <svg class="dropdown-icon {{ $business->is_active ? 'text-danger' : 'text-success' }} me-2" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                            @if($business->is_active)
                ...
                if (!noResultsRow) {
                    ...
                    noResultsRow.innerHTML = `
                        <td colspan="8" class="text-center py-5">
                            ...
                            <small class="text-muted">Intenta buscar por otro nombre, email o RFC</small>
                        </td>
                    `;
